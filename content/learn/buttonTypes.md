...
### A button element accepts three values for the type attribute:

type=submit. The button submits the parent form when clicked. The form gathers its values, fires the submit event, and dispatches the request described by the form's action and method attributes. This is the default if the type attribute is missing.

type=reset. The button restores every form control to its initial value. It does not submit anything. Rarely used in modern interfaces because users often click it by accident.

type=button. The button does nothing on its own. It stays inactive until JavaScript adds a click handler. This is the right choice for any interactive control that is not a form submission.
