...
- `useFont()` - plain composable. Family + size persisted via VueUse `useLocalStorage`. Family is written via VueUse `useCssVar('--al-font')`; size is written via direct `documentElement.style.fontSize` (it's not a custom property). On mount it detects which system fonts the device cannot render (`isFontAvailable`, canvas metrics) and exposes `availableFontOptions` with those filtered out; if the saved font needs one the device lacks, it resets to the default. See "Font availability detection" below.
...
### Font availability detection (current branch)

The "Comic Sans" choice in the font picker was the only option that relied on a system font rather than a self-hosted one. Its old stack ended in `cursive`, so on devices without Comic Sans MS (iOS, Android) it resolved to a looping script face (iOS Snell Roundhand) - unreadable, and the exact opposite of the dyslexia-friendly intent. Two layers now fix this:

- **Detect and hide.** `app/utils/isFontAvailable.ts` measures canvas text widths to tell whether a named system font is actually installed (a rendered width that shifts when the font is named means it took over; identical means it fell back). This is more reliable than `document.fonts.check()`, which does not track installed system fonts consistently. A `FontOption` can now declare `requiresSystemFont`; `useFont` detects unavailable ones on mount and exposes `availableFontOptions` with them removed, so the picker never offers a choice that would render as an unreadable fallback. Comic Sans therefore disappears from the picker on iOS/Android while genuine Comic Sans MS (and its real dyslexia benefits) stays for Windows/macOS users who have it. Detection runs after mount, so the server render and first client paint both show the full list - no hydration mismatch. If a user's saved font needs a now-unavailable system font, `useFont` resets them to the default rather than leaving them on the fallback.
- **Self-hosted net.** Comic Sans MS is proprietary and cannot be bundled, but `@fontsource/comic-neue` (SIL OFL) is a faithful open revival. It is imported in `main.css` and slotted into the stack as `"Comic Sans MS", "Comic Sans", "Comic Neue", sans-serif`, so even if a button ever leaked through onto a device without Comic Sans MS, it renders a Comic-style face instead of plain `sans-serif`. `cursive` is gone entirely.

Tests: `test/unit/is-font-available.test.ts` (canvas detection via a stubbed context), `test/unit/display-control-options.test.ts` (the `requiresSystemFont` metadata + the pure `detectUnavailableSystemFonts` / `filterAvailableFonts` helpers), and two added cases in `test/nuxt/display-controls-panel.test.ts` (the option is hidden when unavailable; the saved font resets to default). Browser caveat: on macOS Comic Sans MS is installed, so neither the hiding nor the Comic Neue fallback is visible in a desktop preview - both need a device that lacks Comic Sans MS to observe.
