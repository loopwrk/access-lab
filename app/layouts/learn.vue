 ...
 *
 * Distinct from the studio's `default` layout: no AppSidebar, no
 * PreviewToolbar, no AppInspector, no CodeDrawer, no MobileBlocker.
 * This is a reading surface, deliberately quiet. Just the AppBar
 * (kept so the font / contrast / dark-mode controls still apply
 * while reading), a categorised tree aside on desktop, and the
 * article slot filled by `pages/learn/[topicId].vue`.
 *
 * Mobile: the tree collapses behind a slideover toggled by a
 * hamburger in the local toolbar. No MobileBlocker — this layout is
 * deliberately mobile-usable.
 */
const { t } = useI18n();
