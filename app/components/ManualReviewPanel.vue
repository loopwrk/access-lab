...
<template>
  <div class="flex flex-col gap-3">
    <div v-if="totalCount" class="flex items-center justify-between">
      <p class="text-(length:--al-font-size-heading) font-semibold text-(--text-primary) m-0">
        {{ t('manual.title') }}
      </p>
      <UBadge :color="progressColor" variant="soft" size="md">
        {{ t('manual.progress', { checked: checkedCount, total: totalCount }) }}
      </UBadge>
    </div>

    <div v-if="totalCount" class="flex gap-2">
      <UButton size="md" variant="outline" color="neutral" @click="checkAll()">
        {{ t('manual.checkAll') }}
      </UButton>
      <UButton size="md" variant="outline" color="neutral" @click="uncheckAll()">
        {{ t('manual.uncheckAll') }}
      </UButton>
    </div>
    ...
    <p v-if="!totalCount" class="text-(length:--al-font-size-body) text-(--text-muted) m-0 leading-normal">
      {{ t('manual.empty') }}
    </p>

    <div v-else class="flex flex-col gap-2">
      <UCheckbox v-for="item in items" :key="item.id" :model-value="item.checked" variant="card" color="neutral"
        @update:model-value="setChecked(item.id, !!$event)">
        <template #label>
          <div class="flex flex-col gap-0.5">
            <span v-if="item.wcagSc"
              class="text-(length:--al-font-size-caption) font-semibold text-(--text-muted) uppercase tracking-[0.06em]">
              WCAG {{ item.wcagSc }}
            </span>
            <span v-if="item.source === 'axe'"
              class="text-(length:--al-font-size-heading) font-semibold text-(--text-primary)">
              {{ formatRuleId(item.title) }}
            </span>
            <span v-else class="text-(length:--al-font-size-heading) font-semibold text-(--text-primary)">
              {{ item.title }}
            </span>
          </div>
        </template>
        <template #description>
          <div class="flex flex-col gap-1">
            ...
            <a v-if="item.helpUrl" :href="item.helpUrl" target="_blank" rel="noopener noreferrer"
