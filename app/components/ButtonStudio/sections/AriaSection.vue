...
<template>
  <fieldset class="flex flex-col gap-3 border-0 p-0 m-0">
    ...
    <UFormField class="flex flex-col mb-4">
      ...
      <UInput :model-value="model.ariaLabel ?? ''" :placeholder="t('controls.ariaLabelPlaceholder')" class="w-full"
        @update:model-value="update('ariaLabel', String($event))" />
    </UFormField>

    <UCheckbox v-if="!hideDisabled" :model-value="model.disabled === true" variant="card" color="primary" size="md"
      :ui="CONTROL_CARD_UI" @update:model-value="update('disabled', $event === true)">
      <template #label>
        {{ t('controls.disabled') }}
      </template>
    </UCheckbox>
