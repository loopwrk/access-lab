...
<template>
  <fieldset class="flex flex-col gap-3 border-0 p-0 m-0">
    <legend class="control-group-title mb-1.5">
      <a href="#topic-switches" class="control-label-link" @click.prevent="focusLearnTopic('switches')">
    ...
    <UFieldGroup size="sm" orientation="vertical">
      <UButton v-for="opt in OPTIONS" :key="opt.value" :color="behaviour === opt.value ? 'primary' : 'neutral'"
        :variant="behaviour === opt.value ? 'solid' : 'ghost'" @click="update('switchBehaviour', opt.value)">
        {{ t(opt.labelKey) }}
      </UButton>
    </UFieldGroup>

    <UFormField v-if="behaviour !== 'none' && supportsPillStyling" class="flex flex-col mt-2">
      <template #label>
        <span class="control-group-title">{{ t('controls.switchPillStyling') }}</span>
      </template>
      <USwitch :model-value="model.switchPillStyling !== false" size="sm" color="primary"
