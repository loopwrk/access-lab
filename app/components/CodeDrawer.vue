...
const maxHeightPx = computed(() => Math.round((windowHeight.value * MAX_HEIGHT_VH) / 100));
  ...
  <div>
    <div
      ...
      :class="[
        ...
        isDragging ? 'bg-(--brand)' : 'bg-(--border) hover:bg-(--brand) focus-visible:bg-(--brand)',
      ]"
    ...
    <UCollapsible
      v-model:open="isOpen"
      :class="['bg-(--surface)', isOpen ? '' : 'border-t border-(--border)']"
    >
      <UButton
        color="neutral"
        variant="ghost"
        block
        size="sm"
        leading-icon="i-lucide-code"
        trailing-icon="i-lucide-chevron-down"
        class="group justify-start font-semibold px-3"
        :ui="{
          trailingIcon:
            'ms-auto group-data-[state=open]:rotate-180 transition-transform duration-200',
        }"
      >
        {{ t("codeDrawer.title") }}
      </UButton>
      <template #content>
        <div class="flex flex-col gap-3 px-5 pb-4 pt-4 border-t border-(--border)">
          <UFieldGroup
            size="xs"
          ...
          <!--
            Copy actions are contextual to the active tab. With no CSS, inline
            and class output are identical, so HTML shows a single Copy; once any
            CSS exists (studio defaults or class-converted inline styles) it gets
            the inline-vs-classes split. CSS and JS get their own single copy.
          -->
          <div
            v-if="codeView === 'html'"
          ...
          >
            <UButton
              ...
              variant="outline"
              color="neutral"
              :icon="copied === 'inline' ? 'i-lucide-check' : 'i-lucide-copy'"
              :disabled="!renderedHtml"
              @click="copyContent('inline')"
            >
              {{ copied === "inline" ? t("codeDrawer.copied") : t("codeDrawer.copy") }}
            </UButton>
            <template v-else>
              <UButton
                class="min-w-[110px] flex justify-center"
              ...
              <UTooltip :text="t('codeDrawer.copyClassesTooltip')">
                <UButton
                  ...
                  variant="outline"
                  color="neutral"
                  :icon="copied === 'class' ? 'i-lucide-check' : 'i-lucide-copy'"
                  :disabled="!renderedHtml"
          ...
          >
            <UButton
              ...
              variant="outline"
              color="neutral"
              :icon="copied === 'css' ? 'i-lucide-check' : 'i-lucide-copy'"
              :disabled="!hasCss"
              @click="copyContent('css')"
            >
              {{ copied === "css" ? t("codeDrawer.copied") : t("codeDrawer.copyCss") }}
            </UButton>
          </div>
          <div
          ...
          >
            <UButton
              ...
              variant="outline"
              color="neutral"
              :icon="copied === 'js' ? 'i-lucide-check' : 'i-lucide-copy'"
              :disabled="!hasJs"
              @click="copyContent('js')"
            >
              {{ copied === "js" ? t("codeDrawer.copied") : t("codeDrawer.copyJs") }}
            </UButton>
          </div>
        </div>
      </template>
