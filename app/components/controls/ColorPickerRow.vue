...
<template>
  <ColorPicker v-slot="{ show }" v-model="model" with-alpha with-initial-color with-eye-dropper with-hex-input
    with-rgb-input>
    <div class="flex items-center justify-between gap-3">
      <button type="button" :class="SWATCH_CLASS" :disabled="disabled" @click="show">
        <div :class="SWATCH_INNER_CLASS" :style="{ backgroundColor: model }" />
      </button>
      <div class="flex flex-col flex-1 min-w-0">
        <span :class="LABEL_TITLE_CLASS">{{ label }}</span>
        <span :class="LABEL_HEX_CLASS">{{ model }}</span>
      </div>
      <UInput v-model="model" size="sm" :disabled="disabled" class="w-24 shrink-0" />
    </div>
  </ColorPicker>
</template>
