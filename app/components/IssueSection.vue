...
<template>
  <UCollapsible :open="isOpen" @update:open="onOpenChange">
    <UButton variant="ghost" :color="color" block size="md" class="justify-between pl-2" :label="label"
      trailing-icon="i-lucide-chevron-down"
      :ui="{ trailingIcon: 'group-data-[state=open]:rotate-180 transition-transform duration-200' }" />

    <template #content>
      <div v-if="violations.length === 0" class="text-(length:--al-font-size-body) text-(--text-muted) m-0 py-2 pl-2">
        {{ emptyMessage }}
      </div>
          ...
          <template #header>
            <div class="flex flex-col gap-1 max-h-min p-0">
              <div class="flex flex-wrap items-center gap-1.5 mb-1">
                <UBadge :label="formatRuleId(violation.id)" :color="isPass ? 'success' : impactColor(violation.impact)"
            ...
            <UCollapsible :default-open="false">
              ...
              <template #content>
                <div class="flex flex-col gap-2 pt-2 text-(--text-secondary) leading-normal">
                  <div v-if="whyText(violation.tags)"
