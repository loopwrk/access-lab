...
export const contentOverflow: DomRule = {
  ...
  evaluate(m) {
    ...
    const parts: string[] = [];
    if (overflowX) {
      parts.push(`content width ${m.scrollWidth}px exceeds container ${m.clientWidth}px`);
    }
    if (overflowY) {
      parts.push(`content height ${m.scrollHeight}px exceeds container ${m.clientHeight}px`);
    }
